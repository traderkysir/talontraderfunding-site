...
==========================================
Connects to Rithmic and AUTO-DISCOVERS every account on the connection.
No account_id needed — Rithmic hands them all back on login.

Setup:
  1. Edit talon_config.json (no account_id required)
  2. Double-click start_talon_server.bat
...
talon_config.json:
{
  ...
  "system_name": "Rithmic 01",
  "server_url":  "rituz00100.rithmic.com:443",
  "firm_name":   "BluSky"
}
"""
...
def load_config():
    ...
    if missing:
        logger.error(f"Config missing: {missing}")
        return None
    return cfg
...
# ── State ────────────────────────────────────────────────────────────────────
conn_state = {"connected": False, "connecting": False,
              "error": None, "firm_name": "", "last_update": None}

# All accounts discovered from Rithmic — { account_id: data_dict }
accounts = {}

# Which account the dashboard is currently showing
selected_id = None
...
def _new_acct(acct_id, acct_name=""):
    return {
        ...
        "balance": 0.0, "equity": 0.0, "starting_balance": 0.0,
        "day_pnl": 0.0, "day_closed_pnl": 0.0, "open_pnl": 0.0,
        "net_qty": 0, "altv": 0.0, "daily_dd_used": 0.0,
        "margin_balance": 0.0, "buying_power": 0.0,
        "equity_history": [], "pnl_history": [], "fills": [],
        "streaming": False, "last_update": None,
    }
...
def setup_pnl_handlers(client):
    ...
    async def on_instrument_pnl(r):
        acct_id = getattr(r, "account_id", "")
        if not acct_id: return
        if acct_id not in accounts:
            accounts[acct_id] = _new_acct(acct_id)
            logger.info(f"Discovered via stream: {acct_id}")
            _push_list()
        a = accounts[acct_id]
    ...
    async def on_account_pnl(r):
        ...
        if acct_id not in accounts:
            accounts[acct_id] = _new_acct(acct_id)
            logger.info(f"Discovered via account pnl: {acct_id}")
            _push_list()
        a = accounts[acct_id]
        ...
        if a["starting_balance"] == 0.0 and bal > 0:
            ...
            logger.info(f"[{acct_id}] Starting balance: ${bal:,.2f}")

        a.update(balance=bal, equity=bal+opnl, day_pnl=dpnl,
                 day_closed_pnl=cpnl, open_pnl=opnl, net_qty=nqty,
                 margin_balance=mar, buying_power=bpow, streaming=True,
    ...
    logger.info("PnL handlers registered (all accounts)")

def setup_fill_handler(client):
    async def on_fill(r):
        acct_id = getattr(r, "account_id", "")
        if not acct_id or acct_id not in accounts: return
        fill = {"time":   datetime.now().strftime("%H:%M:%S"),
...
async def _discover(client):
    ...
    try:
        ...
        for a in plant.accounts:
            ...
            found.append(aid)
            logger.info(f"Account discovered: {aid}  ({aname})")
...
async def _connect(cfg):
    ...
    try:
        client = RithmicClient(
            ...
            app_name="TalonTraderFunding", app_version="1.0.0",
            url=cfg["server_url"],
        ...
        # Free unused plants
        for pname in ["ticker","history"]:
            try:
                p = client.plants.get(pname)
...
# ── Background poller ─────────────────────────────────────────────────────────
def _poll_loop():
    FAST, SLOW, last = 0.5, 5.0, 0.0
    while True:
        time.sleep(0.1)
        if not conn_state["connected"] or _client is None: continue
        now = time.time()
        has_pos = any(d["net_qty"] != 0 for d in accounts.values())
        if now - last < (FAST if has_pos else SLOW): continue
        last = now
        try:
            plant = _client.plants.get("order")
            if not plant or not plant.accounts: continue
            ...
            for ra in raccts:
                ...
                if aid not in accounts:
                    ...
                    _push_list()
                try:
                    sums = run_async(_client.list_account_summary() if len(raccts)==1
                                     else _client.list_account_summary(account_id=aid))
                    for s in (sums or []):
                        if getattr(s, "account_id", "") != aid: continue
                        a = accounts[aid]
                        ...
                        if a["starting_balance"] == 0.0 and bal > 0:
                            a["starting_balance"] = bal
                        a.update(
                            balance=bal, equity=bal+opnl,
                            day_pnl=safe_float(getattr(s,"day_pnl",0)),
                            day_closed_pnl=safe_float(getattr(s,"closed_position_pnl",0)),
                            open_pnl=opnl,
                            net_qty=int(getattr(s,"net_quantity",0) or 0),
                            margin_balance=safe_float(getattr(s,"margin_balance",0)),
                            buying_power=safe_float(getattr(s,"available_buying_power",0)),
                            last_update=datetime.now().isoformat())
            ...
            logger.debug(f"Poll loop: {e}")

threading.Thread(target=_poll_loop, daemon=True, name="PnLPoller").start()

# ── Routes ────────────────────────────────────────────────────────────────────
@app.route("/")
def root(): return send_from_directory(".", "dashboard.html")
...
def api_accounts():
    return jsonify([
        {"account_id": aid, "account_name": d["account_name"],
         ...
         "day_pnl": d["day_pnl"], "open_pnl": d["open_pnl"],
         "net_qty": d["net_qty"], "altv": d["altv"],
         "streaming": d["streaming"], "last_update": d["last_update"]}
        for aid, d in accounts.items()])
...
def api_summary():
    ...
    return jsonify({"connected": conn_state["connected"], "account_id": selected_id,
...
def api_disconnect():
    ...
    if _client:
        try: run_async(_client.disconnect())
        except: pass
        _client = None
    accounts.clear()
    selected_id = None
...
def on_snapshot():
    if selected_id: _push(selected_id)
    _push_list()
